...
import { Eye, EyeOff, ArrowRight, ArrowLeft, Phone, Mail } from "lucide-react";
...
function LoginForm() {
  ...
  const [confirmFn, setConfirmFn] = useState<((otp: string) => Promise<any>) | null>(null);
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast({ type: "error", title: "Login failed", message: error.message || "Invalid email or password" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      showToast({ type: "error", title: "Google sign-in failed", message: error.message || "Please try again" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendOtp = async () => {
    if (!phoneNumber || phoneNumber.length < 10) {
      showToast({ type: "error", title: "Invalid phone", message: "Enter a valid phone number" });
      return;
    }
    if (!recaptchaVerifierRef.current) {
      showToast({ type: "error", title: "reCAPTCHA not ready", message: "Please wait and try again" });
      return;
    }
    setIsLoading(true);
    try {
      ...
      showToast({ type: "error", title: "Failed to send OTP", message: error.message || "Please try again" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast({ type: "error", title: "Verification failed", message: error.message || "Invalid OTP" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-neutral-500">Sign in to access your account and orders</p>
      </div>
        ...
        <div className="space-y-4">
          {!otpSent ? (
            <>
              ...
              />
              <div ref={recaptchaRef} className="flex justify-center my-4" />
              <Button onClick={handleSendOtp} variant="primary" size="xl" fullWidth isLoading={isLoading} rightIcon={<ArrowRight className="w-4 h-4" />}>
                Send OTP
              </Button>
            </>
          ) : (
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button variant="outline" size="lg" fullWidth onClick={handleGoogleSignIn} disabled={isLoading}>
