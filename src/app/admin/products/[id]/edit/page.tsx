...
const mockProduct = {
  ...
  categoryId: "mango",
  description: "Traditional Andhra-style mango pickle made with raw mangoes and authentic spices.",
  ingredients: "Raw mango, mustard oil, red chili powder, fenugreek, turmeric, salt",
  spiceLevel: "medium",
  shelfLifeDays: 365,
  processDescription: "Sun-dried for 3 days with traditional spice blend",
  isActive: true as boolean,
  featured: true as boolean,
};

const mockVariants: VariantEntry[] = [
  { weightGrams: 200, price: 149, stockQuantity: 50, sku: "MANGO-200" },
  { weightGrams: 500, price: 299, stockQuantity: 45, sku: "MANGO-500" },
  { weightGrams: 1000, price: 549, stockQuantity: 20, sku: "MANGO-1000" },
];
...
export default function EditProductPage() {
  ...
  const [variants, setVariants] = useState<VariantEntry[]>(mockVariants);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    showToast({ type: "success", title: "Product updated", message: `${product.name} has been saved` });
    setIsSaving(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-heading font-semibold text-neutral-900">Edit Product</h1>
          <p className="text-sm text-neutral-500 mt-1">Updating: {product.name}</p>
        </div>
      </div>
